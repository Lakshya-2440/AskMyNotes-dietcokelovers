...
export function Transparency() {
  ...
  return (
    <section 
      id="how-it-works" 
      ref={targetRef} 
      className="relative h-[300vh] text-paleivory"
      style={{ backgroundColor: '#2A2529', color: '#F3F0E7' }}
    >
      {/* Sticky container that stays in view while we scroll vertically */}
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        
        <div className="absolute top-24 left-20 md:left-24 z-10 w-full max-w-xl">
          <h2 className="text-4xl md:text-5xl font-serif text-paleivory mb-4 text-balance">
            Complete Transparency.
          </h2>
          <p className="text-lg text-paleivory/70 text-balance">
        ...
        <motion.div style={{ x }} className="flex gap-8 px-12 md:px-24 mt-20 pb-20 pt-44">
          {PIPELINE_STEPS.map((step, index) => {
            ...
            return (
              <div 
                key={index}
                className="group relative flex h-[360px] w-[320px] shrink-0 flex-col justify-between rounded-2xl bg-paleivory/5 border border-paleivory/10 p-8 shadow-paper backdrop-blur-sm transition-all hover:bg-paleivory/10"
              >
                <div>
                  <div className="mb-8 inline-flex h-14 w-14 items-center justify-center rounded-xl bg-paleivory/10 text-paleivory">
                    <Icon className="h-6 w-6" />
                  </div>
                   <h3 className="mb-3 font-mono tracking-tight text-xl font-medium text-paleivory">{step.title}</h3>
                  <p className="text-sm leading-relaxed text-paleivory/60 group-hover:text-paleivory/80 transition-colors">
                    {step.desc}
                  </p>
                </div>
                ...
                {index < PIPELINE_STEPS.length - 1 && (
                  <div className="absolute top-1/2 -right-8 w-8 flex items-center justify-center pointer-events-none opacity-20">
                    <div className="w-full h-px bg-paleivory" />
                    <div className="absolute right-0 w-2 h-2 rotate-45 border-t border-r border-paleivory" />
                  </div>
                )}
              </div>
            )
